...
class ParrotDroneEnv(robot_gazebo_env.RobotGazeboEnv):
    ...
    def __init__(self):
        ...
        Sensor Topic List:
        * /ardrone/down_camera/image_raw: RGB Camera facing down.
        * /ardrone/front_camera/image_raw: RGB Camera facing front.
        * /ardrone/imu: IMU of the drone giving acceleration and orientation relative to world.
        * /ardrone/sonar: Sonar readings facing front
        * /ardrone/gt_pose: Get position and orientation in Global space
        * /ardrone/gt_vel: Get the linear velocity , the angular doesnt record anything.
        
        Actuators Topic List: 
        * /cmd_vel: Move the Drone Around when you have taken off.
        * /ardrone/takeoff: Publish into it to take off
        * /ardrone/land: Publish to make ParrotDrone Land
    ...
    def _check_imu_ready(self):
        self.imu = None
        rospy.logdebug("Waiting for /ardrone/imu to be READY...")
        while self.imu is None and not rospy.is_shutdown():
            try:
                self.imu = rospy.wait_for_message("/ardrone/imu", Imu, timeout=5.0)
                rospy.logdebug("Current/ardrone/imu READY=>")

            except:
                rospy.logerr("Current /ardrone/imu not ready yet, retrying for getting imu")

        return self.imu
        
    def _check_sonar_ready(self):
        self.sonar = None
        rospy.logdebug("Waiting for /ardrone/sonar to be READY...")
        while self.sonar is None and not rospy.is_shutdown():
            try:
                self.sonar = rospy.wait_for_message("/sonar_height", Range, timeout=5.0)
                rospy.logerr("Current/ardrone/sonar READY=>")

            except:
                rospy.logerr("Current /ardrone/sonar not ready yet, retrying for getting sonar")
    ...
    # Depth camera subscriber check function added by Utsav
    def _check_depth_camera_image_raw_ready(self):
        self.depth_camera_image_raw = None
